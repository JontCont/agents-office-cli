...
- The GUI gains an Agents management panel accessible via a tab or sidebar, backed by REST API (`GET /api/agents`, `POST /api/agents`, `PUT /api/agents`, `DELETE /api/agents`) that reads, writes, updates, and deletes agents in `agent-office.yaml`.
- The Go struct `AgentConfig` in `pkg/config/config.go` is updated with `json` tags to prevent agent fields displaying as `undefined` in the GUI.
...
- `agent-management`: Create, list, update, and delete agents via CLI (`agent create`) and GUI REST API (`GET/POST/PUT/DELETE /api/agents`), persisting changes to `agent-office.yaml`.
